    ...
    Statement = [
      {
        ...
        Resource = aws_ecr_repository.jokejolt.arn
      }
    ]
  })
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:${var.k8s_namespace}:${var.project_name}-app"
          }
        }
      }
    ]
